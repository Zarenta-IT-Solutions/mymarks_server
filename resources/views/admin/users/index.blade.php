        ...
        $('#dataTable').DataTable({
            ...
            columns: [
                ...
                {data: 'action', name: 'action', orderable: false, searchable: false, render:function (data,type,full,meta) {
                        console.log(full.id)
                        let id = full.id;
                        return ' <a href="{{route('admin.users.index')}}/'+id+'/edit" class="bg-indigo-500 text-white active:bg-indigo-600 text-xs font-bold uppercase px-3 py-1 rounded outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150" ><i class="fa fa-edit"></i> Edit</a>';
                    }}
            ]
        });
    </script>
